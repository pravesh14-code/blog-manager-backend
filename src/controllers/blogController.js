...
exports.getAllSaved = async (req, res, next) => {
  try {
    ...
    const formattedBlogs = blogs
      .filter(saved => saved.post && saved.post.author)
      .map(saved => ({
        ...saved.post,
        user: {
...
exports.remove = async (req, res, next) => {
  try {
    await blogService.remove(parseInt(req.params.id));
